...
namespace TutzApp.Services
{
    public class ProcessMonitorService : IProcessMonitorService
    {
        ...
        // NoMoreBorder: backoff quando o helper admin está indisponível
        private const long BorderlessHelperRetryDelayMs = 60000;
        private int _borderlessHelperFailures;
        private long _lastBorderlessHelperFailureTick;
        ...
        // NoMoreBorder: para cada app cadastrado detectado no snapshot deste tick,
        // pede ao helper admin (via BORDERLESS_ENSURE) para aplicar/mantê-lo borderless.
        private void CheckBorderlessApps(HashSet<string> runningProcesses)
        {
            var apps = _sysControl.Config.NoMoreBorder?.Apps;
            if (apps == null || apps.Count == 0)
            {
                return;
            }

            // Se o helper admin falhou recentemente, evita tentar (e possivelmente
            // abrir UAC) a cada tick: revalida no máximo 1x por minuto.
            if (_borderlessHelperFailures > 0 &&
                Environment.TickCount64 - _lastBorderlessHelperFailureTick < BorderlessHelperRetryDelayMs)
            {
                return;
            }
            ...
            {
                if (rule == null || string.IsNullOrWhiteSpace(rule.ProcessName))
                {
                    continue;
                }

                if (!runningProcesses.Contains(rule.ProcessName))
                {
                    continue;
                }
                ...
                try
                {
                    string response = _sysControl.ApplyBorderlessNow(rule);
                    if (response.StartsWith("ERROR", StringComparison.OrdinalIgnoreCase))
                    {
                        if (_borderlessHelperFailures == 0)
                        {
                            _sysControl.LogDebug($"ProcessMonitor: BORDERLESS_ENSURE falhou para '{rule.ProcessName}': {response}");
                        }
                        _borderlessHelperFailures++;
                        _lastBorderlessHelperFailureTick = Environment.TickCount64;
                    }
                    else if (_borderlessHelperFailures != 0)
                    {
                        _borderlessHelperFailures = 0;
                        _sysControl.LogDebug("ProcessMonitor: helper admin voltou a responder para NoMoreBorder.");
                    }
                }
                catch (Exception ex)
                {
                    _borderlessHelperFailures++;
                    _lastBorderlessHelperFailureTick = Environment.TickCount64;
                    _sysControl.LogDebug($"ProcessMonitor: exceção ao aplicar NoMoreBorder em '{rule.ProcessName}': {ex.Message}");
                }
            }
        }
        ...
        private void EnsureNormalProfileIfSupported(string reason)
        {
            if (_sysControl is SystemControlService concreteSystemControl)
            {
                concreteSystemControl.EnsureNormalProfileIfMouseSpeedIsNotNormal(reason);
            }
            else
            {
                // Dublês de teste e implementações alternativas podem não expor leituras Win32/HID.
                // Mantém no-op para não obrigar mocks a conhecer detalhes de SPI/mouse.
                _sysControl.LogDebug($"ProcessMonitor: validação de perfil normal ignorada para implementação {_sysControl.GetType().Name}.");
            }
        }
        ...
        private async Task CheckProcessesAsync(CancellationToken cancellationToken)
        {
            ...
            // NoMoreBorder: aplica/mantém janelas borderless dos apps cadastrados
            // usando o mesmo snapshot de processos deste tick.
            CheckBorderlessApps(runningProcesses);
            ...
            if (_gamesRunningState < 0 && !anyGameNow)
            {
                _gamesRunningState = 0;
                _sysControl.LogDebug("ProcessMonitor: estado inicial sem jogo detectado; validando perfil normal pelo Windows mouse speed.");
                EnsureNormalProfileIfSupported("startup sem jogo detectado");
                return;
            }
