...
namespace TutzApp.ViewModels
{
    public partial class MainViewModel : ObservableObject
    {
        ...
        [ObservableProperty]
        private string _forceKillProtectedProcessesText = string.Empty;

        [ObservableProperty]
        private int _doublePressIntervalMs = 300;

        [ObservableProperty]
        private int _gamepadMouseSensitivityPercent = 67;

        [ObservableProperty]
        private bool _suppressWindowsControllerVkMapping = true;

        [ObservableProperty]
        private bool _ownGuideButton = true;

        [ObservableProperty]
        private int _guideHoldThresholdMs = 700;

        [ObservableProperty]
        private bool _mapGamepadToKeyboardInGameBar = true;

        [ObservableProperty]
        private bool _launchFocusAssist = true;

        [ObservableProperty]
        private bool _playniteBridgeEnabled = true;
        ...
        public ObservableCollection<RecognizedGamepad> RecognizedGamepads { get; } = new();
        public ObservableCollection<VibranceAppRule> VibranceAppRules { get; } = new();
        public ObservableCollection<BorderlessAppRule> BorderlessApps { get; } = new();
        ...
        public System.Collections.Generic.List<string> SystemActions { get; } = new() { "VirtualKeyboard", "ToggleKeyboardProfile", "PowerOffMonitor", "ToggleDisplayTarget", "ToggleGamepadHelp", "ToggleGamepadMouseMode", "VolumeDown", "VolumeUp", "ShowGameBar", "Screenshot", "ForceKillForegroundApp" };
            ...
            INvidiaVibranceService nvidiaVibrance)
        {
            ...
            GamepadMouseSensitivityPercent = Config.Gamepad.MouseModeSensitivityPercent;
            MonitoredGamesText = string.Join(", ", Config.Monitoring.Games);
            ForceKillProtectedProcessesText = string.Join(", ", Config.ForceKill.ProtectedProcesses);
            ...
            foreach (var rule in Config.NoMoreBorder.Apps)
            {
                BorderlessApps.Add(rule);
            }
            ...
            if (Config.GameConsole != null)
            {
                SuppressWindowsControllerVkMapping = Config.GameConsole.SuppressWindowsControllerVkMapping;
                OwnGuideButton = Config.GameConsole.OwnGuideButton;
                GuideHoldThresholdMs = Config.GameConsole.GuideHoldThresholdMs;
                MapGamepadToKeyboardInGameBar = Config.GameConsole.MapGamepadToKeyboardInGameBar;
                LaunchFocusAssist = Config.GameConsole.LaunchFocusAssist;
                PlayniteBridgeEnabled = Config.GameConsole.PlayniteBridgeEnabled;
            }

            RefreshRecognizedGamepads();
        ...
        private sealed class NoopGamepadMonitorService : IGamepadMonitorService
        {
            ...
            public void StartConnectionStatusMonitoring(System.Threading.CancellationToken cancellationToken) { }
        ...
        private void AutoSaveNoMoreBorderSettings()
        {
            if (_isInitializing || _isSaving) return;
            SaveNoMoreBorderSettings();
        }
        ...
        partial void OnForceKillProtectedProcessesTextChanged(string value) => AutoSaveAutomationSettings();
        partial void OnDoublePressIntervalMsChanged(int value) => AutoSaveAutomationSettings();
        partial void OnGamepadMouseSensitivityPercentChanged(int value) => AutoSaveAutomationSettings();
        partial void OnSuppressWindowsControllerVkMappingChanged(bool value) => AutoSaveAutomationSettings();
        partial void OnOwnGuideButtonChanged(bool value) => AutoSaveAutomationSettings();
        partial void OnGuideHoldThresholdMsChanged(int value) => AutoSaveAutomationSettings();
        partial void OnMapGamepadToKeyboardInGameBarChanged(bool value) => AutoSaveAutomationSettings();
        partial void OnLaunchFocusAssistChanged(bool value) => AutoSaveAutomationSettings();
        partial void OnPlayniteBridgeEnabledChanged(bool value) => AutoSaveAutomationSettings();
        ...
        private void RefreshRecognizedGamepads()
        {
            Action action = () =>
            {
                RecognizedGamepads.Clear();
                if (Config.Gamepad?.RecognizedDevices == null)
                {
                    return;
                }

                foreach (var device in Config.Gamepad.RecognizedDevices)
                {
                    RecognizedGamepads.Add(device);
                }
            };
        ...
        [RelayCommand]
        private void CalibrateGamepad()
        {
            if (!_gamepadMonitor.StartCalibration())
            {
                _sysControl.SetStatusMessage("Não foi possível iniciar a calibração HID.");
            }
        }

        [RelayCommand]
        private void DeleteGamepadCalibration(RecognizedGamepad? device)
        {
            var devices = Config.Gamepad?.RecognizedDevices;
            ...
            var confirmation = System.Windows.MessageBox.Show(
                $"Excluir a calibração de {device.DisplayName}?\n\nO controle deixará de funcionar nos atalhos até ser calibrado novamente.",
                "Excluir calibração de gamepad",
            ...
            {
                RecognizedGamepads.Remove(device);
                _gamepadMonitor.RefreshSettings();
                _sysControl.SetStatusMessage("Calibração de gamepad excluída.");
                return;
            }

            devices.Insert(index, device);
            _sysControl.SetStatusMessage("Não foi possível excluir a calibração de gamepad.");
        }
        ...
        [RelayCommand]
        private void AddBorderlessApp()
        {
            var dialog = new Microsoft.Win32.OpenFileDialog
            {
                Title = "Selecionar aplicativo para NoMoreBorder",
                Filter = "Executáveis (*.exe)|*.exe",
                CheckFileExists = true
            };
            ...
            string exePath = dialog.FileName;
            if (BorderlessApps.Any(rule => rule.ExecutablePath.Equals(exePath, StringComparison.OrdinalIgnoreCase)))
            {
                _sysControl.SetStatusMessage("Aplicativo já cadastrado no NoMoreBorder.");
                return;
            }

            var picker = new Views.BorderlessAppDialogWindow(exePath);
            picker.ShowDialog();
            if (!picker.Confirmed)
            {
                return;
            ...
            var rule = new BorderlessAppRule
            {
                DisplayName = string.IsNullOrWhiteSpace(picker.AppName)
                    ? Path.GetFileNameWithoutExtension(exePath)
                    : picker.AppName.Trim(),
                ExecutablePath = exePath,
                ProcessName = Path.GetFileNameWithoutExtension(exePath),
                MonitorDevice = picker.SelectedMonitorDevice,
                XOffset = picker.XOffset,
                YOffset = picker.YOffset,
                Width = picker.TargetWidth,
                Height = picker.TargetHeight,
                IconPath = ExtractExecutableIcon(exePath, "NoMoreBorderIcons")
            };

            Config.NoMoreBorder.Apps.Add(rule);
            BorderlessApps.Add(rule);
            AutoSaveNoMoreBorderSettings();

            // Se o app já estiver rodando, aplica borderless imediatamente.
            _sysControl.ApplyBorderlessNow(rule);
        }

        [RelayCommand]
        private void EditBorderlessApp(BorderlessAppRule? rule)
        {
            if (rule == null) return;

            var editor = new Views.BorderlessAppDialogWindow(rule);
            editor.ShowDialog();
            if (!editor.Confirmed)
            {
                return;
        ...
        [RelayCommand]
        private void RemoveBorderlessApp(BorderlessAppRule? rule)
        {
            if (rule == null) return;

            Config.NoMoreBorder.Apps.Remove(rule);
            BorderlessApps.Remove(rule);
            AutoSaveNoMoreBorderSettings();
        }

        [RelayCommand]
        private void SaveNoMoreBorderSettings()
        {
            if (!TrySave(() =>
                {
                    Config.NoMoreBorder.Apps = BorderlessApps.ToList();
                }))
            {
                _sysControl.SetStatusMessage("Falha ao salvar NoMoreBorder.");
            }
        }
        ...
        private static string ExtractExecutableIcon(string exePath, string? directoryName = null)
        {
            try
            {
                string iconDirectory = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, directoryName ?? "VibranceIcons");
                Directory.CreateDirectory(iconDirectory);

                // Regras por caminho completo (NoMoreBorder) usam um hash do caminho
                // no nome do arquivo para evitar colisão entre executáveis homônimos.
                string fileName = Path.GetFileNameWithoutExtension(exePath);
                if (directoryName != null)
                {
                    using var sha = System.Security.Cryptography.SHA256.Create();
                    string hash = Convert.ToHexString(sha.ComputeHash(System.Text.Encoding.UTF8.GetBytes(exePath)));
                    fileName = $"{fileName}_{hash[..8]}";
                }

                string iconPath = Path.Combine(iconDirectory, $"{fileName}.png");
        ...
        private void SaveAutomationSettings()
        {
            if (TrySave(() =>
            {
                ...
                DoublePressIntervalMs = Config.Monitoring.DoublePressIntervalMs;
                Config.Gamepad.MouseModeSensitivityPercent = Math.Clamp(GamepadMouseSensitivityPercent, 10, 200);
                GamepadMouseSensitivityPercent = Config.Gamepad.MouseModeSensitivityPercent;

                Config.GameConsole.SuppressWindowsControllerVkMapping = SuppressWindowsControllerVkMapping;
                Config.GameConsole.OwnGuideButton = OwnGuideButton;
                Config.GameConsole.GuideHoldThresholdMs = Math.Clamp(GuideHoldThresholdMs, 200, 3000);
                GuideHoldThresholdMs = Config.GameConsole.GuideHoldThresholdMs;
                Config.GameConsole.MapGamepadToKeyboardInGameBar = MapGamepadToKeyboardInGameBar;
                Config.GameConsole.LaunchFocusAssist = LaunchFocusAssist;
                Config.GameConsole.PlayniteBridgeEnabled = PlayniteBridgeEnabled;
                ...
                var protectedProcesses = new System.Collections.Generic.List<string>();
                if (!string.IsNullOrWhiteSpace(ForceKillProtectedProcessesText))
                {
                    var parts = ForceKillProtectedProcessesText.Split(new char[] { ',', ';', '\r', '\n', '\t' }, StringSplitOptions.RemoveEmptyEntries);
                    foreach (var part in parts)
                    {
                        string clean = Path.GetFileName(part.Trim());
                        if (!string.IsNullOrEmpty(clean) &&
                            !protectedProcesses.Any(existing => existing.Equals(clean, StringComparison.OrdinalIgnoreCase)))
                        {
                            protectedProcesses.Add(clean);
                        }
                    }
                }

                if (!protectedProcesses.Any(process => process.Equals("explorer.exe", StringComparison.OrdinalIgnoreCase)))
                {
                    protectedProcesses.Insert(0, "explorer.exe");
                }

                Config.ForceKill.ProtectedProcesses = protectedProcesses;
                ForceKillProtectedProcessesText = string.Join(", ", Config.ForceKill.ProtectedProcesses);
